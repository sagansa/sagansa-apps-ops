...
export default function ReceiptConfigContent() {
    const { user } = useAuth();
    const t = useTranslations('POS.receiptConfig');
    ...
    const loadStores = async () => {
        try {
            ...
            setError(err instanceof Error ? err.message : t('failedLoadStores'));
            setStores([]);
        } finally {
            setLoading(false);
        }
        ...
        return (
            <div className="flex min-h-[400px] items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="mt-2 text-sm text-gray-600">{t('loadingStores')}</p>
                </div>
            </div>
        );
    }
