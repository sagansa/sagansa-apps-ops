...
import { Attendance, AttendanceStatus } from '@/app/services/api';
...
export default function AttendanceList({ attendances, onStatusUpdate }: AttendanceListProps) {
  ...
  const formatDateTime = (dateString: string | null) => {
    ...
    return new Date(dateString).toLocaleString(locale === 'en' ? 'en-US' : 'id-ID', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
            ...
            <tr key={attendance.id} className="hover:bg-gray-50">
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <div className="text-sm font-medium text-gray-900">
                      {attendance.creator?.name || t('table.unknown')}
                    </div>
                    <div className="text-sm text-gray-500">
                      {attendance.creator?.email || t('table.na')}
                    </div>
                  </div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-gray-900">
                  {attendance.store?.name || t('table.na')}
                </div>
                <div className="text-sm text-gray-500">
                  {attendance.store?.nickname || t('table.noNickname')}
                </div>
                {attendance.store?.radius !== null && attendance.store?.radius !== undefined && (
                  <div className="text-xs text-gray-400">
                    {t('table.radius', { n: attendance.store.radius })}
                  </div>
                )}
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-gray-900">
                  {attendance.shiftStore?.name || t('table.na')}
                </div>
